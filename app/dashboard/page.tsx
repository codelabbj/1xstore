...
import { ArrowDownToLine, ArrowUpFromLine, Wallet, Loader2, Send, Download, Ticket, MessageCircleMore, Clock, CheckCircle2, XCircle, AlertCircle, ChevronRight, Gift, Lock } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const depositGateRef = useRef<HTMLDivElement>(null)
  ...
  // Close deposit gate when clicking outside
  useEffect(() => {
    if (!showDepositGate) return
    const handleOutsideClick = (e: MouseEvent) => {
      if (depositGateRef.current && !depositGateRef.current.contains(e.target as Node)) {
        setShowDepositGate(false)
      }
    }
        ...
        <div className="relative group">
          ...
          >
            <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-2.5 sm:p-5 border border-slate-200 dark:border-slate-800 hover:border-violet-500/50 hover:shadow-lg hover:shadow-violet-500/10 transition-all h-full flex flex-col items-center sm:items-start">
              <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-lg sm:rounded-2xl bg-gradient-to-br from-violet-400 to-purple-600 flex items-center justify-center mb-1.5 sm:mb-3 group-hover:scale-110 transition-transform shadow-lg shadow-amber-500/20 relative">
                {isCheckingDeposit ? (
          ...
          {/* Deposit gate message */}
          {showDepositGate && (
            <div
              ref={depositGateRef}
              className="absolute z-50 top-full left-1/2 -translate-x-1/2 mt-2 w-64 bg-white dark:bg-slate-900 rounded-2xl border border-violet-200 dark:border-violet-800 shadow-xl shadow-violet-500/10 p-4 text-center"
            >
              <div className="w-10 h-10 rounded-xl bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center mx-auto mb-3">
              ...
              <Link
                ...
                className="block w-full py-2 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white text-xs font-semibold hover:opacity-90 transition-opacity"
                onClick={() => setShowDepositGate(false)}
              >
                Faire un dépôt
              </Link>
            </div>
            ...
            <div>
              {recentTransactions.map((transaction, index) => {
                ...
                return (
                  <div
                    key={transaction.id}
                    className={`flex items-center gap-4 p-4 ${index !== recentTransactions.length - 1 ? 'border-b border-slate-100 dark:border-slate-800' : ''
                      }`}
                  >
                    {/* Icon */}
